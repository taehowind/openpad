...
type BoardListRow = {
  ...
  owner_id: string | null; audience: BoardAudience; access_password_hash: string | null; type: BoardType; background: string; created_at: string; updated_at: string;
...
function toSummary(row: BoardListRow): BoardSummary {
  return {
    ...
    audience: row.audience, requirePassword: Boolean(row.access_password_hash),
...
export async function POST(request: Request) {
  ...
  const passwordHash = parsed.data.accessPassword ? hashPassword(parsed.data.accessPassword) : null;
  const timestamp = now();
  (await transaction(async () => {
    (await run(
      `INSERT INTO boards (id, title, description, share_token, share_code, share_mode, owner_id, audience, access_password_hash, type, background, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      boardId, parsed.data.title, parsed.data.description, shareToken, shareCode, parsed.data.shareMode,
      session.id, parsed.data.audience, passwordHash, parsed.data.type, normalizeBackground(parsed.data.background),
      timestamp, timestamp,
    ));
