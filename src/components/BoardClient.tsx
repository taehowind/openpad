...
export function BoardClient({ identifier, mode }: BoardClientProps) {
  ...
  async function copyShareLink() {
    try {
      ...
      await navigator.clipboard.writeText(`${window.location.origin}/j/${data.board.shareCode}`);
  ...
  async function copyShareCode() {
    try {
      if (!data?.board.shareCode) throw new Error("복사할 참여 코드가 없습니다.");
      await navigator.clipboard.writeText(data.board.shareCode);
  ...
  async function attachFile(boardId: string, form: FormData, file: File) {
    try {
      const ticket = await fetch(`/api/boards/${boardId}/uploads`, {
        method: "POST", headers: { "Content-Type": "application/json" },
      ...
      // 501 = this deployment uploads through the API; anything else is a real failure.
      if (ticket.status !== 501) throw new Error(await responseError(ticket, "업로드를 준비하지 못했습니다."));
    } catch (reason) {
      if (reason instanceof Error && reason.message.includes("스토리지")) throw reason;
      // Network hiccup reaching the ticket route — fall back to posting the file.
    }
  ...
  async function setBoardAccessPassword() {
    ...
    const value = await dialog.prompt({
      ...
      label: "입장 비밀번호",
      placeholder: data.board.requirePassword ? "새 비밀번호 (비우면 해제)" : "비밀번호 입력",
      confirmLabel: "저장",
      required: false,
      maxLength: 100,
    });
  ...
  if (!data) return null;
  const shareUrl = data.board.shareCode ? `${window.location.origin}/j/${data.board.shareCode}` : "";
